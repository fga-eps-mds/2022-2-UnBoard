...
@pytest.mark.parametrize(
    ("parametro", "resultado_esperado"), [(2019, 0), (2022, 0), (2023, 0)]
)
def test_se_tamanho_de_cursos_por_ano_eh_0_se_passar_nenhum_curso(
...
@pytest.mark.xfail
@pytest.mark.parametrize(
    ("parametro", "resultado_esperado"), [(2019, 1), (2022, 1), (2023, 1)]
)
def test_se_tamanho_de_cursos_por_ano_eh_1_se_passar_nenhum_curso(
    parametro, resultado_esperado
):
    search_bar = Courses_list(parametro)
    resultado = len(search_bar.search_course(""))
...
def test_se_tamanho_de_cursos_por_ano_eh_dif_de_0_se_passar_nenhum_curso(
    parametro, resultado_esperado
):
    search_bar = Courses_list(parametro)
    resultado = len(search_bar.search_course(""))
...
@pytest.mark.parametrize(("parametro"), [2019, 2022, 2023])
